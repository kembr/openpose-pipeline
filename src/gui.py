...
def create_plot_window():
    ...
    def show():
        ...
        fig.suptitle("")  # Title?
        if chosen_joint1.get() != "Select a Joint": plot_joint_over_time(
            list(JOINTS.keys())[list(JOINTS.values()).index(chosen_joint1.get())], data, axis=ax1)
        if chosen_joint2.get() != "Select a Joint": plot_joint_over_time(
            list(JOINTS.keys())[list(JOINTS.values()).index(chosen_joint2.get())], data, axis=ax2)
    ...
    choose_joint_frame1.pack(padx=5, pady=10)
